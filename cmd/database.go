...
func updateAllDatabaseTablesStructure(c *core.CliContext) error {
	...
	err = datastore.Container.UserDataStore.SyncStructs(new(models.Product))

	if err != nil {
		return err
	}

	log.BootInfof(c, "[database.updateAllDatabaseTablesStructure] product table maintained successfully")

	err = datastore.Container.UserDataStore.SyncStructs(new(models.ProductAlias))

	if err != nil {
		return err
	}

	log.BootInfof(c, "[database.updateAllDatabaseTablesStructure] product alias table maintained successfully")
